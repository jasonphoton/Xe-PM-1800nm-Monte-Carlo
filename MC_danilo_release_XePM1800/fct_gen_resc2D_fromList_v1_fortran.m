function [vx_resc_list vy_resc_list vz_resc_list te_resc_list tr_resc_list tt_resc_list vr_resc_list th_resc_list trind_resc_list] = fct_gen_resc2D_fromList_v1_fortran(winkel_total,A,IonAmp,tgrid,original_rescind,rescind,trind_mat,nsample_resc,sigwp,sigtg,PlotOpt)
% sigwp cross-section wavepacket dispersion
% sigtg cross-section wavepacket dispersion

% Max.M. version rewritten to accept numerical cross sections from fortran

%% programm geändert, das anglegrid wird vom fortran greenpot programm übernommen!!
...
anglegrid  = deg2rad(winkel_total); % achtung, bei fortran ist es in DEG
...
for m=1:NrMax
    ...
    for l=1:length(vrm)
        Wthm(l) = (sum(sigtg(vrm(l),anglegrid)));

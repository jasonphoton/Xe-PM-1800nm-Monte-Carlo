function Main_loop_save_v7_spineffects_only_hist_1dot8mu_with_qprop
% close all
%  clear all

% pulse characteristics
n        = 10;                                   % propto pulse duration, (cos^2)
N        = 0;                                   % propto flat part
wvlm     = 7000e-9;                            %
atom     = 'He1s2s-singlet'; % %atom     = 'He1s2s-singlet';    %'He1s2s-triplet'
s        = 3/2;
pulse    = [n N wvlm];
dt       = 0.1;               % temporaresolution
kmax     = 100;              
% number of cep/int settings for vol avg
nsample  = 4e7;             % number of particles per cep/int setting
...
display(strcat('k loop ',num2str(k),' done!'));
   ...
   % %     savename2    = [num2str(k),'_all',savestr,'.mat'];
    save(savename2,...
         'vpagrid_01','vpegrid_01','hist_vyvz_resc_loop_01');
end
toc
end
...
%% define the field
% % % [Env E I A ALPHA BETA v r tgrid E_fh] = fct_get_EnvEIAAlphBetvr_sin2_nN(dt,wvlm,IWcm,CEP,n/2,N);

ww=omega/(2.0*n);
duration=n*2*pi/omega;
phi_cep = CEP;
timeee  = 0:dt:duration;
E0    =  sqrt(IWcm/3.509e16);

A = E0/omega*(sin(ww*timeee)).^2.*sin(omega*timeee+phi_cep); 
E = -diff(A)./diff(timeee);
E(length(timeee))=0;
tgrid = timeee;
...
% qp_field_y = E0/omega*(sin(ww*time)).^2.*sin(omega*time+phi_cep); 
% plot(timeee,qp_field_x)
...
% Z    = sc(1);        % Xe 54.00, Ar 18,    Ne 10
% mu   = sc(2);        % Xe 3.604,  Ar 2.727, Ne 2.038
% rcov = rc;
% sigtg=@(q,theta) (((2.*Z).^2))./((mu^.2 + 4.*(q.^2).*(sin(0.5.*theta).^2)).^2); % (abs(q).*
...
%eliminate slow returning electrons
% % % for m=1:1
% % %         vrm = A(trind_mat(rescind,m))-A(texit_ind(rescind));
% % %         index = find(abs(vrm)<0.1);
% % %         rescind(index) = [];
% % % end
